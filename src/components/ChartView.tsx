...
export default function ChartView({ stats }: ChartViewProps) {
  ...
  const maxTime = Math.max(...topRoutes.map(s => s.maxTime));
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6">
      ...
      <div className="space-y-4">
        {topRoutes.map((stat, index) => {
          const percentage = (stat.maxTime / maxTime) * 100;
            ...
            <div key={stat.route} className="group">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <span className="flex-shrink-0 w-6 h-6 bg-red-100 rounded-md flex items-center justify-center text-xs font-semibold text-red-600">
                    {index + 1}
                  </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col text-right">
                      ...
                      <span className="text-sm font-semibold text-red-600">
                        {stat.maxTime >= 1000
                          ? `${(stat.maxTime / 1000).toFixed(2)}s`
                          : `${stat.maxTime.toFixed(0)}ms`}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div
                  className={`h-full transition-all duration-700 ease-out rounded-lg ${
                    isError
                      ...
                      : stat.maxTime > maxTime * 0.7
                      ? 'bg-gradient-to-r from-red-400 to-red-500'
                      : stat.maxTime > maxTime * 0.4
